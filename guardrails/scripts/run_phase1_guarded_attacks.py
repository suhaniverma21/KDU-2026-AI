...
def main() -> None:
    ...
    with httpx.Client(base_url="http://127.0.0.1:8000", timeout=10.0) as client:
        for prompt in ATTACK_PROMPTS:
            response = client.post("/chat-guarded", json={"message": prompt})
            ...
            results.append(
                {
                    ...
                    "guardrails_backend": payload["guardrails_backend"],
                    "input_guardrail_ms": payload["input_guardrail_ms"],
                    "output_guardrail_ms": payload["output_guardrail_ms"],
